...
complete -c nnn -s b -r -d 'bookmark key to open' -x -a '(echo $NNN_BMS | awk 
-F: -v RS=\; \'{print $1"\t"$2}\')'
...
complete -c nnn -s P -r -d 'plugin key to run' -x -a '(echo $NNN_PLUG | awk 
-F: -v RS=\; \'{print $1"\t"$2}\')'
...
complete -c nnn -s s -r -d 'load session by name' -x -a '@\t"last session" (ls 
$sessions_dir)'
